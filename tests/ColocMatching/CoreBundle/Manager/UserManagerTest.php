...
class UserManagerTest extends TestCase {
	...
	public function testGetUserById() {
		...
		$this->userManager->expects($this->once())->method("getById")->with($mockedUser->getId())->willReturn($mockedUser);
	
		/** @var User $user */
		$user = $this->userManager->getById($mockedUser->getId());
	...
	public function testGetUsers() {
		...
		$this->userManager->expects($this->once())->method("getAll")->with($this->isInstanceOf(AbstractFilter::class))->willReturn($mockedUsers);
		$this->userManager->expects($this->once())->method("countAll")->willReturn(count($mockedUsers));
	
		$users = $this->userManager->getAll(new UserFilter());
	...
	public function testGetFieldFromOneUser() {
		/** @var array */
		...
		$this->userManager->expects($this->once())->method("getFieldsById")->with($mockedUser->getId(), $fields)
		->willReturn(array (
				"id" => $mockedUser->getId(),
				"email" => $mockedUser->getEmail(),
				"lastname" => $mockedUser->getLastname(),
				"gender" => $mockedUser->getGender()
		));
			
		$user = $this->userManager->getFieldsById(1, $fields);
